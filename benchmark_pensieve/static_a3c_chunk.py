...
class ActorNetwork(object):
	...
	def create_actor_network(self):
		with tf.variable_scope('actor'):
			...
			split_3 = tflearn.conv_1d(inputs[:, 3:4, :], 128, 5, activation='relu')			# buffer size
			split_4 = tflearn.conv_1d(inputs[:, 4:5, :], 128, 5, activation='relu')			# buffer size
			split_5 = tflearn.fully_connected(inputs[:, 5:6, -1], 8, activation='relu')		# player state 0 or 1
			split_6 = tflearn.fully_connected(inputs[:, 6:7, -1], 8, activation='relu')	# server wait
			split_7 = tflearn.fully_connected(inputs[:, 7:8, -1], 8, activation='relu')	# freezing
			...
			merge_net = tflearn.merge([split_0_flat, split_1_flat, split_2_flat, split_3_flat, split_4_flat, split_5, split_6, split_7], 'concat')
