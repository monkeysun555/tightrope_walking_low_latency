...
class Live_Player(object):
    ...
    def get_future_trace(self, lh_window_size):
        # Get current time and return furture average bandwidth for each segment downloading
        future_bw = []
        curr_time_ratio = self.last_trace_time % 1.0
        first_ratio = 1.0 - curr_time_ratio 
        second_ratio = curr_time_ratio
        curr_time_idx = self.time_idx
        future_time_idx = [((curr_time_idx+i)%len(self.throughput_trace),(curr_time_idx+i+1)%len(self.throughput_trace)) for i in range(lh_window_size)]
        for i in future_time_idx:
            future_bw.append((first_ratio*self.throughput_trace[i[0]]+second_ratio*self.throughput_trace[i[1]]))
        return future_bw
