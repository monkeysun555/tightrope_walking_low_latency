...
def main():
    ...
    if massive:
        ...
        if random_latency:
            compare_file = open(compare_path + 'stallion.txt' , 'w')
        else:
            compare_file = open(compare_path + 'stallion' + str(int(init_latency)) +'s.txt' , 'w')
 
        solver = stallion.stallion_solver()
        while True:
            # Start testing
            ...
            solver.update_tp_latency(ave_bw, latency)
            total_reward = 0.0
            while not env.streaming_finish():
                if env.get_player_state() == 0:
                    ...
                    action_1, action_2 = solver.solve(tmp_buffer, tmp_latency)
